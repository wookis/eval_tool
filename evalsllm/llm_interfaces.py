...
class OpenAILLM(LLMInterface):
    ...
    def generate_response(self, prompt: str) -> Any:
        ...
        try:
            ...
            logger.error(f"Error2 calling OpenAI API: {e}")
            #return None

    def get_model_name(self) -> str:
        return self.model_name

class KT_MAGMA_PRD_LLM(LLMInterface):
...
class KT_MAGMA_DEV_LLM_BASE_2_3_2(LLMInterface):
    """KT_MAGMA_DEV_LLM_BASE_2_3_2의 상호작용을 위한 클래스"""
    def __init__(self, model_name: str, api_key: Optional[str] = None):
        self.api_key = api_key or os.getenv("KT_MAGMA_DEV_gemma_API_KEY")
        if not self.api_key:
            raise ValueError("KT_MAGMA_DEV_gemma_API_KEY가 설정되지 않았습니다.")
